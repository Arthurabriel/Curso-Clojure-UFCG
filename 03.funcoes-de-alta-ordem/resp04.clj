(defn somador [operador]
  (fn [lista]
    (loop [sequencia lista acumulador 0]
        (if (empty? sequencia)
                acumulador
                    (recur (rest sequencia) (int (+ acumulador (operador (first sequencia)))))))))

(def soma-cubos
    (somador (fn [x] (Math/pow x 3))))
...
(def soma-pares
    (somador (fn [x] (if (even? x) x 0)))) 
                           
(def conta
  (somador (fn [x] (Math/pow x 0))))

(assert (= (conta (range 100000000)) 100000000))
